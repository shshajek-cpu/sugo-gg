...
// GET: Fetch premium content records for a character and date
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch premium content records
    const { data, error } = await supabase
      .from('ledger_premium_content')
      .select('*')
    ...
    console.error('Error in GET /api/ledger/premium-content:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}

// POST: Create or update premium content record
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to save premium content' }, { status: 500 })
    }

    return NextResponse.json(data)
  } catch (error) {
    console.error('Error in POST /api/ledger/premium-content:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
