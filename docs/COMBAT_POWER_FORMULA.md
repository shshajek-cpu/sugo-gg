...
PVE와 동일한 기본 공식을 사용하지만, **PVP 전용 스탯**을 적용합니다.

### PVE vs PVP 스탯 차이

| 구분 | PVE | PVP |
|------|-----|-----|
| 추가 공격력 | PVE공격력 + 보스공격력 | PVP공격력 |
| 피해 증폭 | 피해증폭 + PVE피해증폭 + 보스피해증폭 | 피해증폭 + PVP피해증폭 |
| 공격력 증가% | 공격력증가% | 공격력증가% + PVP공격력증가% |

### PVP 공식

```
PVP최종공격력 = (기본공격력 × (1 + (공격력증가% + PVP공격력증가%) / 100)) + PVP공격력

PVP파워멀티플라이어 = 1 + 치명타계수 + (피해증폭 + PVP피해증폭)/100 + 치명타피해증폭/100 + 강타/100

PVP전투력 = (PVP공격력계수 × PVP파워멀티플라이어 × 전투효율 × 정확도보정 × 다단히트계수) × 1000 × 12.03
```

### 특징
- **보스 관련 스탯 제외**: 보스공격력, 보스피해증폭은 PVP에 미적용
- **PVP 전용 스탯 추가**: PVP공격력증가%, PVP피해증폭% 적용
- 나머지 계수 (전투효율, 정확도보정, 다단히트)는 PVE와 동일
...
| 캐릭터 | 클래스 | 아이템레벨 | PVE 전투력 | PVP 전투력 | PVE 등급 |
|--------|--------|-----------|-----------|-----------|----------|
| 도룡뇽 | 살성 | 2,823 | 11,276 | (새로고침 필요) | G3 |
| 죄수 | 수호성 | 3,093 | 24,530 | (새로고침 필요) | Sa5 |
| 대화 | 마도성 | - | 42,171 | (새로고침 필요) | R4 |
| 혜지성 | 치유성 | 3,120 | 37,454 | (새로고침 필요) | R1 |
| 차은욱 | 검성 | 3,170 | 38,431 | (새로고침 필요) | R2 |
| 아담 | 검성 | 3,076 | 35,216 | (새로고침 필요) | R5 |
| 하와 | 치유성 | 3,020 | 21,890 | (새로고침 필요) | E4 |

> **참고**: PVP 전투력을 확인하려면 사이트에서 캐릭터 상세 페이지 → "새로고침" 버튼 클릭
